...
let logger = new (winston.Logger)({
    transports : [
        new (winston.transports.File)({ filename : __dirname+'/log/careerlink.log' })
    ]
})
    ...
    return new Promise(function(fulfill,reject){
        ...
        ic.queue({
            ...
            callback : function(error,result,$){
                ...
                let tag = [];
                $('.job-data').children('p').children().each(function(index,elem){
                    tag.push($(elem).text());
                })
        ...
        if(data.logo_company_url !=''){
           var link  = {
            ...
            rp(link).then(function(response,html){
                ...
                .pipe(fs.createWriteStream(img_Path+slug(data.job_company_info.company_name,{lower : true})+'.'+ext))
                .on('finish',function(){
                ...
                data['logo_company_url'] = img_Path+slug(data.job_company_info.company_name,{lower : true})+'.'+ext;
                fulfill(data)
            })
        }else{
            fulfill(data);
